  ...
  return (
    <div className="flex flex-col items-center justify-center gap-6">
      ...
      <div className="flex flex-col items-center gap-1.5">
        <span className="text-[10px] font-black uppercase tracking-[0.5em] text-[#53C8FF] animate-pulse italic">Initializing Cloud</span>
        <div className="flex gap-1.5">
           {[0, 1, 2].map(i => (
             <div key={i} className="w-1 h-1 bg-[#53C8FF]/40 rounded-full animate-bounce" style={{ animationDelay: `${i * 150}ms` }}></div>
           ))}
        </div>
      </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .logo-path {
          animation: draw-logo 3s cubic-bezier(0.4, 0, 0.2, 1) infinite;
        }
        @keyframes draw-logo {
